...
如实写出来，因为**关于验证范围的声明本身也是一条声明**。

**靠真跑验证的**

- 预发布规则，对着 node-semver 自己的答案核：17 个用例，其中包括社区列表贡献指南给出的那条"修复方案"——
  实测它**并不能**匹配 `0.1.5-rc.2`。
- 检出能力，对着两个真实已发布插件与三个**为违反特定规则而专门构造**的夹具。`dsh-market` 零 findings；
  夹具都被如实报出。
- CLI：真实运行、`--json`、以及退出码（`0` / `1` / `2`）。
- 发布产物：打包、解包，并把 10 个文件与 `files[]` 逐项核对。
- 宿主半侧：装进 profile，并在**真实运行的会话里**调用 `plugin_preflight`，返回的 findings 数量与 CLI 一致。
  报告渲染器也在同一个测试里跑过。

**靠构造保证、而非测试覆盖的**

- semver 范围引擎是针对本检查需要而**重写的最小实现**，不是移植版。它覆盖本工具用到的运算符
  （`^`、`~`、比较符、连字符区间、`||`、通配符）；它不是通用 semver 实现，也没打算是。
